...
## 1.7.6: Now on macOS, and a brand refresh
- **Now on your Mac:** a new section on lazymonster.space for the Apple Silicon edition, with its install line, what runs where, and the new 38-second film.
- **The films:** "Now on macOS" joins the original and the vertical cut.
- **Party mode:** the monster on the site runs through its skins, outfits and moods on the beat.
- **README refresh:** an animated hero, Windows and Mac edition cards, a moods-and-skins strip, new icons and a Mac teaser. All art is generated by `tools/make_readme_art.py`.
