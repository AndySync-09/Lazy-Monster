...
- **Seeing the screen.** Only when you ask a question about it. The front window's text, plus any text read from its screenshot on your PC (OCR on the NPU), goes to your chosen brain for that one answer; the screenshot itself goes only to a local brain, or to a cloud brain if you turn that on in Settings > Privacy. Nothing is saved to disk; windows that look private (passwords, recovery codes, keys, banking) are refused before anything is captured.
