...
RULES: List[Rule] = [
    ...
    ("set_reminder", _r(r"remind me ((?:at|in) [\w: ]+?|tomorrow(?: morning| afternoon| evening| at [\w: ]+?)?|tonight(?: at [\w: ]+?)?) (?:to|about|that) (.+)"),
     lambda m: {"what": m.group(2), "when": m.group(1)}),
    ("set_reminder", _r(r"remind me (?:to |about |that )?(.+?) ((?:at|in) .+|tomorrow.*|tonight.*|this (?:evening|afternoon).*)"),
     lambda m: {"what": m.group(1), "when": m.group(2)}),
    ("set_reminder", _r(r"(?:at|in) (.+?),? remind me (?:to |about |that )?(.+)"),
     lambda m: {"what": m.group(2), "when": ("at " if not re.match(r"\d+ (min|hour)", m.group(1)) else "in ") + m.group(1)}),
    ("list_reminders", _r(r"(?:what are|list|tell me) my reminders|(?:do i have )?any reminders"), lambda m: {}),
    ("cancel_reminder", _r(r"(?:cancel|delete|remove|forget) (?:the |my )?(.+?) reminder"), lambda m: {"what": m.group(1)}),
