...
It finds or installs Python, installs Lazy-Monster in its own folder, asks for your OpenAI key (optional), downloads the voice models, learns your voice, and starts it in the background. Run it again to update. Prefer to read it first? See [install.ps1](install.ps1). To remove: `uninstall.ps1` in `%LOCALAPPDATA%\LazyMonster\app`.
